...
int main()
{
	...
	char datoteka[MAX_SIZE];
	strcpy(datoteka, "drzava.txt");
...
int Unos(Pos new, char* drzavadat) {
	...
	while (!feof(dat)) {
		...
		newg = createelement(imegrada, brojstanovnika);
		insert(new->root, newg);
	}
...
{
	Pos temp = first->next;
	while (strcmp(newperson->drzava, temp->next->drzava) > 0)
	{
		temp = temp->next;
	}
